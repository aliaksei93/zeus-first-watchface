export const LAYOUT = {
  designWidth: 480,
  frame: {
    dividerX: 45,
    dividerWidth: 390,
    dividerHeight: 3,
    dividerY: [164, 313],
  },
  alarm: {
    icon: { x: 55, y: 125, w: 28, h: 28 },
    text: { x: 92, y: 127, w: 72, h: 24, size: 24 },
  },
  battery: {
    enabled: true,
    icon: { x: 279, y: 407, w: 28, h: 28 },
  },
  weather: {
    text: { x: 335, y: 127, w: 51, h: 24, size: 24 },
    icon: { x: 395, y: 124, w: 30, h: 30 },
  },
  sun: {
    icon: { x: 174, y: 407, w: 28, h: 28 },
    text: { x: 211, y: 410, w: 59, h: 22, size: 22 },
  },
  clock: {
    time: { x: 84, y: 195, w: 312, h: 90, size: 90 },
    period: { x: 398, y: 191, w: 32, h: 21, size: 20 },
    seconds: { x: 394, y: 261, w: 40, h: 24, size: 24 },
    date: { x: 169, y: 65, w: 143, h: 26, size: 26 },
    weekday: { x: 158, y: 330, w: 164, h: 32, size: 26 },
  },
  interactions: {
    calendar: { x: 151, y: 53, w: 178, h: 50 },
    schedule: { x: 146, y: 320, w: 188, h: 52 },
    battery: { x: 275, y: 397, w: 37, h: 48 },
    weather: { x: 325, y: 114, w: 110, h: 50 },
    sun: { x: 164, y: 397, w: 110, h: 48 },
    alarm: { x: 45, y: 114, w: 129, h: 50 },
    alarmShortcut: { x: 84, y: 195, w: 112, h: 90 },
    countdown: { x: 284, y: 195, w: 110, h: 90 },
    stopwatch: { x: 394, y: 249, w: 43, h: 46 },
  },
};
